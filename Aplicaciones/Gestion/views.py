...
def _construir_queryset_prodvet(request):
    ...
    if stock_bajo == '1':
        prodvet_query = prodvet_query.filter(stock_pv__lte=10)  # stock bajo si es <= 10
    ...
    if orden == 'reciente':
        prodvet_query = prodvet_query.order_by('-created_at_pv')
    elif orden == 'nombre':
        prodvet_query = prodvet_query.order_by('nombre_pv')
    elif orden == 'codigo':
        prodvet_query = prodvet_query.order_by('codigo_pv')
    elif orden == 'stock':
        prodvet_query = prodvet_query.order_by('stock_pv')
    elif orden == 'vencimiento':
        prodvet_query = prodvet_query.order_by('fecha_vencimiento_pv')
    else:
        prodvet_query = prodvet_query.order_by('-created_at_pv')

    return prodvet_query
...
def listadoprodvet(request):
    ...
    try:
        ...
        total_stock_bajo = ProductoVeterinario.objects.filter(stock_pv__lte=10).count()
        ...
        stock_bajo_filtrado = prodvet_query.filter(stock_pv__lte=10).count()
        ...
        return render(request, 'catalogos/salud/prodvet/lista_prodvet.html', {
            ...
            'total_activos': 0,
            'total_vacunas': 0,
            'total_antibioticos': 0,
            'total_vitaminas': 0,
            'total_stock_bajo': 0,
            'total_filtrado': 0,
            'stock_bajo_filtrado': 0,
            'vencidos_filtrado': 0,
            'proveedores': [],
            'tipos_producto': [],
        })
...
def reporteprodvet_imprimir(request):
    ...
    try:
        ...
        stock_bajo = prodvet_query.filter(stock_pv__lte=10).count()
...
def exportarprodvet_excel(request):
    """
    Descarga en Excel (.xlsx) de los productos veterinarios.
    """
    try:
        import openpyxl
        from openpyxl.styles import Font, PatternFill, Alignment
...
def exportarprodvet_csv(request):
    """
    Descarga en CSV de los productos veterinarios.
    """
    try:
        import csv
