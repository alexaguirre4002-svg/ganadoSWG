...
def guardarpotrero(request):
    ...
    try:
        ...
        try:
            # Se usa Decimal (no float) para no arrastrar imprecisión binaria
            # que Django luego rechaza como "más de 2 dígitos decimales"
...
def eliminainseminacion(request, id_in):
    ...
    try:
        ...
        from .models import Prenez  # Importar modelo de preñeces
        if Prenez.objects.filter(fk_in=inseminacion).exists():
...
def dashboard_grafico(request):
    ...
    por AÑO → MES, con TODAS las gráficas de módulo (reproducción,
    sanidad, peso, producción por raza, composición de leche, finanzas)
    calculadas específicamente para cada período.
    ...
    animales_categoria = Animal.objects.values('categoria_an').annotate(cantidad=Count('pk'))
    animales_estado = Animal.objects.values('estado_an').annotate(cantidad=Count('pk'))
    ...
    if total_insem_global and pendientes_insem_global / total_insem_global > 0.3:
        recomendaciones.append({
            'tipo': 'warning', 'icono': 'bi-hourglass-split',
            'titulo': 'Muchas inseminaciones pendientes de resultado',
            'texto': f'El {round(pendientes_insem_global/total_insem_global*100,1)}% de las inseminaciones no tienen resultado confirmado en todo el historial. Actualizar diagnósticos de preñez a tiempo.'
        })

    peso_categoria_global = Animal.objects.exclude(peso_actual_kg_an__isnull=True).values(
        'categoria_an'
    ).annotate(promedio=Avg('peso_actual_kg_an')).order_by('promedio')
    if peso_categoria_global:
        cat_min = peso_categoria_global[0]
        recomendaciones.append({
    ...
    # ==========================================================
    # HISTORIAL DETALLADO POR AÑO → MES CON TODAS LAS GRÁFICAS
    # DE MÓDULO CALCULADAS POR PERÍODO (OPTIMIZADO: GROUP BY)
    # ==========================================================
    meses_es = {
    ...
    top_animal_raw = Ordeno.objects.annotate(
        anio=ExtractYear('fecha_or'), mes=ExtractMonth('fecha_or')
    ).values('anio', 'mes', 'fk_an__codigo_an').annotate(total=Sum('litros_or'))
    ...
    raza_por_periodo = defaultdict(list)
    for r in raza_raw:
        raza_por_periodo[(r['anio'], r['mes'])].append(r)
    ...
    composicion_por_periodo = {(r['anio'], r['mes']): r for r in composicion_raw}
    ...
    insem_detalle_raw = Inseminacion.objects.annotate(
        anio=ExtractYear('fecha_in'), mes=ExtractMonth('fecha_in')
    ).values('anio', 'mes', 'resultado_in').annotate(total=Count('pk'))
    insem_detalle_por_periodo = defaultdict(list)
    for r in insem_detalle_raw:
        insem_detalle_por_periodo[(r['anio'], r['mes'])].append(r)
    ...
    eventos_tipo_por_periodo = defaultdict(list)
    for r in eventos_tipo_raw:
        eventos_tipo_por_periodo[(r['anio'], r['mes'])].append(r)
    ...
    pesaje_cat_por_periodo = defaultdict(list)
    ...
    for anio, mes in todos_los_periodos:
        ...
        raza_p = raza_por_periodo.get(key, [])
        ...
        comp_p = composicion_por_periodo.get(key, {})
        ...
        insem_detalle_p = insem_detalle_por_periodo.get(key, [])
        ...
        eventos_tipo_p = eventos_tipo_por_periodo.get(key, [])
...
def _construir_queryset_pesaje(request):
    """
    Aplica todos los filtros del listado de pesajes sobre el queryset.
    Se usa tanto en el listado como en la exportación/impresión.
    """
    search = request.GET.get('search', '')
    fk_an = request.GET.get('fk_an', '')
    metodo_pe = request.GET.get('metodo_pe', '')
    condicion_corporal = request.GET.get('condicion_corporal', '')
    rango_fecha = request.GET.get('rango_fecha', '')
    fecha_desde = request.GET.get('fecha_desde', '')
    fecha_hasta = request.GET.get('fecha_hasta', '')
    ...
    pesaje_query = Pesaje.objects.all().select_related('fk_an', 'fk_us_pe')
    ...
    if fk_an:
        try:
            pesaje_query = pesaje_query.filter(fk_an_id=int(fk_an))
        except ValueError:
            pass

    if metodo_pe:
        pesaje_query = pesaje_query.filter(metodo_pe=metodo_pe)

    if condicion_corporal:
        try:
            pesaje_query = pesaje_query.filter(condicion_corporal_pe=int(condicion_corporal))
        except ValueError:
            pass

    if peso_min:
        try:
            pesaje_query = pesaje_query.filter(peso_kg_pe__gte=float(peso_min))
        except ValueError:
            pass

    if peso_max:
        try:
            pesaje_query = pesaje_query.filter(peso_kg_pe__lte=float(peso_max))
        except ValueError:
            pass

    hoy = date.today()
    if rango_fecha == 'hoy':
        pesaje_query = pesaje_query.filter(fecha_pe=hoy)
    elif rango_fecha == 'semana':
        inicio_semana = hoy - timedelta(days=hoy.weekday())
        pesaje_query = pesaje_query.filter(fecha_pe__gte=inicio_semana, fecha_pe__lte=hoy)
    elif rango_fecha == 'mes':
        pesaje_query = pesaje_query.filter(fecha_pe__year=hoy.year, fecha_pe__month=hoy.month)
    elif rango_fecha == 'anio':
        pesaje_query = pesaje_query.filter(fecha_pe__year=hoy.year)
    elif rango_fecha == 'personalizado':
        if fecha_desde:
            pesaje_query = pesaje_query.filter(fecha_pe__gte=fecha_desde)
        if fecha_hasta:
            pesaje_query = pesaje_query.filter(fecha_pe__lte=fecha_hasta)

    ordenes_validos = {
        'reciente': '-fecha_pe',
        'antiguo': 'fecha_pe',
        'peso_mayor': '-peso_kg_pe',
        'peso_menor': 'peso_kg_pe',
        'animal': 'fk_an__codigo_an',
    }
    return pesaje_query.order_by(ordenes_validos.get(orden, '-fecha_pe'))


def listapesaje(request):
    """
    Muestra el listado completo de pesajes con filtros avanzados,
    búsqueda, estadísticas y exportación de reportes.
    """
    # ==========================================
    # ESTADÍSTICAS GENERALES
    # ==========================================
    total_pesajes = Pesaje.objects.count()
    
    total_estimacion = Pesaje.objects.filter(metodo_pe='estimacion_visual').count()
    total_cinta = Pesaje.objects.filter(metodo_pe='cinta_metrica').count()
    total_bascula = Pesaje.objects.filter(metodo_pe='bascula').count()

    peso_promedio_general = Pesaje.objects.aggregate(prom=Avg('peso_kg_pe'))['prom'] or 0
    peso_maximo_general = Pesaje.objects.aggregate(max=Max('peso_kg_pe'))['max'] or 0
    peso_minimo_general = Pesaje.objects.aggregate(min=Min('peso_kg_pe'))['min'] or 0
    ...
    # APLICAR FILTROS
    # ==========================================
    pesaje_query = _construir_queryset_pesaje(request)

    total_filtrado = pesaje_query.count()
    peso_promedio_filtrado = pesaje_query.aggregate(prom=Avg('peso_kg_pe'))['prom'] or 0
    peso_maximo_filtrado = pesaje_query.aggregate(max=Max('peso_kg_pe'))['max'] or 0
    peso_minimo_filtrado = pesaje_query.aggregate(min=Min('peso_kg_pe'))['min'] or 0
    ...
    contexto = {
        ...
        'total_cinta': total_cinta,
        'total_bascula': total_bascula,
        'peso_promedio_general': round(peso_promedio_general, 2),
        'peso_maximo_general': round(peso_maximo_general, 2),
        'peso_minimo_general': round(peso_minimo_general, 2),
        'total_filtrado': total_filtrado,
        'peso_promedio_filtrado': round(peso_promedio_filtrado, 2),
        'peso_maximo_filtrado': round(peso_maximo_filtrado, 2),
        'peso_minimo_filtrado': round(peso_minimo_filtrado, 2),
        'animales': animales,
    }
...
def reportepesaje_imprimir(request):
    """
    Vista de solo lectura, sin paginación, con todos los pesajes que
    cumplen los filtros actuales, lista para imprimir.
    """
    try:
        pesaje_query = _construir_queryset_pesaje(request)
        
        peso_promedio = pesaje_query.aggregate(prom=Avg('peso_kg_pe'))['prom'] or 0
        peso_maximo = pesaje_query.aggregate(max=Max('peso_kg_pe'))['max'] or 0
        peso_minimo = pesaje_query.aggregate(min=Min('peso_kg_pe'))['min'] or 0

        return render(request, 'catalogos/animal/pesajes/reporte_pesaje_imprimir.html', {
            'pesajes': pesaje_query,
            'total_resultados': pesaje_query.count(),
            'peso_promedio': round(peso_promedio, 2),
...
def exportarpesaje_excel(request):
    """
    Descarga en Excel (.xlsx) de los pesajes que cumplen los filtros
    actualmente aplicados en el listado.
    """
    try:
        import openpyxl
        ...
        encabezados = [
            'ID', 'Animal', 'Nombre', 'Categoría', 'Fecha',
            'Peso (kg)', 'Condición corporal', 'Método', 'Observaciones', 'Registrado por'
        ]
        hoja.append(encabezados)
        for celda in hoja[1]:
            celda.font = Font(bold=True, color='FFFFFF')
        ...
        for p in pesaje_query:
            # Método con texto
            metodo_texto = ''
            if p.metodo_pe == 'bascula':
                metodo_texto = 'Báscula'
            elif p.metodo_pe == 'cinta_metrica':
                metodo_texto = 'Cinta métrica'
            elif p.metodo_pe == 'estimacion_visual':
                metodo_texto = 'Estimación visual'

            hoja.append([
                p.id_pe,
                p.fk_an.codigo_an if p.fk_an else '',
                p.fk_an.nombre_an if p.fk_an else '',
                p.fk_an.categoria_an if p.fk_an else '',
                p.fecha_pe.strftime('%d/%m/%Y') if p.fecha_pe else '',
                float(p.peso_kg_pe) if p.peso_kg_pe is not None else '',
                p.condicion_corporal_pe if p.condicion_corporal_pe is not None else '',
                metodo_texto,
                p.observaciones_pe or '',
                p.fk_us_pe.username_us if p.fk_us_pe else '',
            ])
        ...
        nombre_archivo = f'pesajes_{date.today().strftime("%Y%m%d")}.xlsx'
        response['Content-Disposition'] = f'attachment; filename="{nombre_archivo}"'
        wb.save(response)
        return response
    except Exception as e:
...
def exportarpesaje_csv(request):
    """
    Descarga en CSV de los pesajes que cumplen los filtros actuales.
    """
    try:
        import csv
        ...
        response = HttpResponse(content_type='text/csv')
        nombre_archivo = f'pesajes_{date.today().strftime("%Y%m%d")}.csv'
        response['Content-Disposition'] = f'attachment; filename="{nombre_archivo}"'
        ...
        for p in pesaje_query:
            ...
            writer.writerow([
                ...
                p.fk_an.categoria_an if p.fk_an else '',
                p.fecha_pe.strftime('%d/%m/%Y') if p.fecha_pe else '',
                p.peso_kg_pe if p.peso_kg_pe is not None else '',
                p.condicion_corporal_pe if p.condicion_corporal_pe is not None else '',
                metodo_texto,
                p.observaciones_pe or '',
                p.fk_us_pe.username_us if p.fk_us_pe else '',
